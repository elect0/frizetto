...
export const actions: Actions = {
	default: async ({ request, locals: { supabase, safeGetSession } }) => {
		...
		const { data: profile } = await supabase
			.from('profiles')
			.select('full_name, email, phone, is_banned, notify_email_confirmation')
		...
		const startTime = new Date(`${form.data.date.split('T')[0]}T${form.data.time}:00+02:00`);
		...
		if (error) {
			return message(
				form,
				'Ai ajuns la limita programărilor active. Te rugăm să finalizezi sau să anulezi una înainte de a adăuga alta.',
				{
					status: 403
				}
			);
		...
		if (profile.notify_email_confirmation) {
			const { data: _emailData, error: _emailError } = await resend.emails.send({
				from: 'programari@frizetto.com',
				to: profile.email,
